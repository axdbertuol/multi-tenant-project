...
class TestAuthEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_verify_token_valid(self, async_client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_logout_all_sessions(self, async_client: AsyncClient):
        ...
        response = await async_client.post("/api/v1/auth/logout", json=logout_data, headers=headers1)
    ...
    async def test_full_auth_flow(self, async_client: AsyncClient):
        ...
        # 4. Verify token
        verify_response = await async_client.post("/api/v1/auth/verify-token", headers=headers)
        assert verify_response.status_code == 200
        
        # 5. Get sessions
        sessions_response = await async_client.get("/api/v1/auth/sessions", headers=headers)
        assert sessions_response.status_code == 200
        
        # 6. Logout
        logout_data = {"revoke_all_sessions": False}
        logout_response = await async_client.post("/api/v1/auth/logout", json=logout_data, headers=headers)
