...
from user.application.use_cases.auth_use_cases import AuthenticationUseCase
...
class TestUserIntegration:
    ...
    def setup_method(self):
        ...
        self.auth_use_case = AuthenticationUseCase(self.mock_uow)
    ...
    def test_complete_user_registration_and_login_flow(self):
        ...
        created_user = User.create(
            email=user_dto.email, name=user_dto.name, password=user_dto.password
        )
    ...
    def test_user_profile_update_flow(self):
        ...
        # Update user profile
        update_dto = UserUpdateDTO(name="Updated Name", is_active=True)

        updated_user = user.update_name(update_dto.name)
        self.mock_user_repository.save.return_value = updated_user
        self.user_use_case._user_domain_service.validate_user_activation.return_value = (
            True,
            "Can be activated",
        )
    ...
    def test_password_change_and_session_invalidation_flow(self):
        ...
        user = User.create(
            email="test@example.com", name="Test User", password=old_password
        )
        ...
        self.auth_use_case._auth_service.revoke_all_user_sessions.assert_called_once_with(
            user_id
        )
    ...
    def test_user_deactivation_and_session_cleanup_flow(self):
        ...
        self.user_use_case._user_domain_service.validate_user_deactivation.return_value = (
            True,
            "Can be deactivated",
        )
    ...
    def test_login_with_invalid_credentials_flow(self):
        ...
        login_dto = LoginDTO(
            email="test@example.com", password="WrongPassword", remember_me=False
        )
    ...
    def test_email_uniqueness_constraint_flow(self):
        ...
        user_dto1 = UserCreateDTO(
            email="duplicate@example.com", name="First User", password="SecurePass123"
        )

        # Mock first registration success
        self.user_use_case._user_domain_service.is_email_available.return_value = True
        created_user = User.create(
            email=user_dto1.email, name=user_dto1.name, password=user_dto1.password
        )
        self.mock_user_repository.save.return_value = created_user
        ...
        with pytest.raises(
            ValueError, match="Email duplicate@example.com is already in use"
        ):
    ...
    def test_remember_me_session_duration_flow(self):
        ...
        login_dto = LoginDTO(
            email="test@example.com", password="SecurePass123", remember_me=True
        )
