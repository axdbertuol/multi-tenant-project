...
from src.authorization.domain.entities.policy import Policy, PolicyCondition, PolicyEffect
...
class TestAuthorizationServiceEdgeCases:
    ...
    def test_authorize_with_policy_evaluation_returning_none(
        ...
        policy = Policy.create(
            ...
            resource_type="document",
            action="read",
            conditions=[PolicyCondition(attribute="user.department", operator="eq", value="engineering")],
            organization_id=sample_context.organization_id,
            created_by=uuid4(),
        )
    ...
    def test_authorize_with_recursive_policy_references(
        ...
        recursive_policy = Policy.create(
            ...
            resource_type="document",
            action="read",
            conditions=[PolicyCondition(attribute="user.id", operator="eq", value="{{user.id}}")],  # Self-reference
            organization_id=sample_context.organization_id,
            created_by=uuid4(),
        )
