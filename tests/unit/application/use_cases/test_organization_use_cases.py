...
class TestOrganizationUseCases:
    ...
    def mock_uow(self):
        """Mock Unit of Work."""
        uow = Mock()
        uow.organizations = Mock()
        uow.users = Mock()
        uow.__aenter__ = Mock(return_value=uow)
        uow.__aexit__ = Mock(return_value=None)
        return uow
    ...
    @pytest.mark.io
     def test_create_organization_success(self, organization_use_cases, mock_uow):
        ...
        created_org = OrganizationFactory.create_organization(
            name="Test Organization", owner_id=owner_id, description="Test description"
        )
        mock_uow.organizations.create.return_value = created_org
        mock_uow.organizations.add_user_to_organization.return_value = None

        # Act
        result = organization_use_cases.create_organization(owner_id, create_dto)
        ...
        mock_uow.organizations.add_user_to_organization.assert_called_once_with(
            created_org.id, owner_id
        )

    @pytest.mark.io
     def test_create_organization_duplicate_name(
        self, organization_use_cases, mock_uow
    ):
        ...
        # Act & Assert
        with pytest.raises(
            ValueError, match="Organization with name Existing Org already exists"
        ):
            organization_use_cases.create_organization(owner_id, create_dto)

    @pytest.mark.io
     def test_create_organization_invalid_owner(
        self, organization_use_cases, mock_uow
    ):
    ...
    @pytest.mark.io
     def test_get_organization_by_id_success(
        ...
        # Act
        result = organization_use_cases.get_organization_by_id(org_id)
    ...
    @pytest.mark.io
     def test_get_organization_by_name_success(
        self, organization_use_cases, mock_uow
    ):
        ...
        # Act
        result = organization_use_cases.get_organization_by_name("Test Org")
    ...
    @pytest.mark.io
     def test_get_organizations_by_owner(self, organization_use_cases, mock_uow):
        ...
        # Act
        result = organization_use_cases.get_organizations_by_owner(owner_id)
    ...
    @pytest.mark.io
     def test_get_user_organizations(self, organization_use_cases, mock_uow):
        ...
        # Act
        result = organization_use_cases.get_user_organizations(user_id)

        # Assert
        assert len(result) == 2

    @pytest.mark.io
     def test_update_organization_success(self, organization_use_cases, mock_uow):
        ...
        original_org = OrganizationFactory.create_organization(
            name="Original Name", owner_id=owner_id
        )
        updated_org = original_org.update_name("Updated Name").update_description(
            "Updated description"
        )
    ...
    @pytest.mark.io
     def test_update_organization_not_owner(
        ...
        with pytest.raises(
            PermissionError,
            match="Only the organization owner can update the organization",
        ):
            organization_use_cases.update_organization(org_id, update_dto, non_owner_id)
    ...
    @pytest.mark.io
     def test_update_organization_duplicate_name(
        ...
        # Act & Assert
        with pytest.raises(
            ValueError, match="Organization with name Existing Name already exists"
        ):
            organization_use_cases.update_organization(org_id, update_dto, owner_id)

    @pytest.mark.io
     def test_transfer_ownership_success(self, organization_use_cases, mock_uow):
        ...
        # Act
        result = organization_use_cases.transfer_ownership(
            org_id, transfer_dto, current_owner_id
        )
    ...
    @pytest.mark.io
     def test_transfer_ownership_not_owner(self, organization_use_cases, mock_uow):
        ...
        # Act & Assert
        with pytest.raises(
            PermissionError, match="Only the current owner can transfer ownership"
        ):
            organization_use_cases.transfer_ownership(
                org_id, transfer_dto, non_owner_id
            )

    @pytest.mark.io
     def test_transfer_ownership_invalid_new_owner(
        self, organization_use_cases, mock_uow
    ):
        ...
        # Act & Assert
        with pytest.raises(
            ValueError, match=f"User with id {new_owner_id} does not exist"
        ):
            organization_use_cases.transfer_ownership(org_id, transfer_dto, owner_id)
    ...
    @pytest.mark.io
     def test_add_user_to_organization_success(
        ...
        # Act
        result = organization_use_cases.add_user_to_organization(
            org_id, add_user_dto, owner_id
        )
    ...
    @pytest.mark.io
     def test_add_user_to_organization_not_owner(
        self, organization_use_cases, mock_uow
    ):
        ...
        # Act & Assert
        with pytest.raises(
            PermissionError, match="Only the organization owner can add users"
        ):
            organization_use_cases.add_user_to_organization(
                org_id, add_user_dto, non_owner_id
            )

    @pytest.mark.io
     def test_add_user_to_organization_already_member(
        self, organization_use_cases, mock_uow
    ):
        ...
        mock_uow.organizations.get_by_id.return_value = org
        mock_uow.users.get_by_id.return_value = user
        mock_uow.organizations.is_user_in_organization.return_value = (
            True  # Already member
        )

        # Act & Assert
        with pytest.raises(
            ValueError, match="User is already a member of this organization"
        ):
            organization_use_cases.add_user_to_organization(
                org_id, add_user_dto, owner_id
            )
    ...
    @pytest.mark.io
     def test_remove_user_from_organization_by_owner(
        self, organization_use_cases, mock_uow
    ):
        ...
        # Act
        result = organization_use_cases.remove_user_from_organization(
            org_id, user_id, owner_id
        )

        # Assert
        assert result is True
        mock_uow.organizations.remove_user_from_organization.assert_called_once_with(
            org_id, user_id
        )

    @pytest.mark.io
     def test_remove_user_from_organization_by_self(
        self, organization_use_cases, mock_uow
    ):
    ...
    @pytest.mark.io
     def test_remove_user_from_organization_cannot_remove_owner(
        self, organization_use_cases, mock_uow
    ):
        ...
        with pytest.raises(
            ValueError,
            match="The organization owner cannot be removed from the organization",
        ):
            organization_use_cases.remove_user_from_organization(
                org_id, owner_id, owner_id
            )

    @pytest.mark.io
     def test_remove_user_not_member(self, organization_use_cases, mock_uow):
        ...
        # Act & Assert
        with pytest.raises(
            ValueError, match="User is not a member of this organization"
        ):
            organization_use_cases.remove_user_from_organization(
                org_id, user_id, owner_id
            )
    ...
    @pytest.mark.io
     def test_deactivate_organization_success(
        self, organization_use_cases, mock_uow
    ):
        ...
        # Act
        result = organization_use_cases.deactivate_organization(org_id, owner_id)
    ...
    @pytest.mark.io
     def test_deactivate_organization_not_owner(
        self, organization_use_cases, mock_uow
    ):
        ...
        with pytest.raises(
            PermissionError,
            match="Only the organization owner can deactivate the organization",
        ):
            organization_use_cases.deactivate_organization(org_id, non_owner_id)

    @pytest.mark.io
     def test_activate_organization_success(
        self, organization_use_cases, mock_uow
    ):
        ...
        # Act
        result = organization_use_cases.activate_organization(org_id, owner_id)
