...
class TestRoleInheritanceEdgeCases:
    ...
    def setup_method(self):
        ...
        self.default_rh_permissions = [
            Permission.create("read_users", "Read user profiles", PermissionAction.READ, "user"),
            Permission.create("update_user_basic", "Update basic user info", PermissionAction.UPDATE, "user_basic"),
            Permission.create("read_organizations", "Read organization data", PermissionAction.READ, "organization"),
            Permission.create("create_reports", "Create basic reports", PermissionAction.CREATE, "report"),
            Permission.create("read_attendance", "Read attendance records", PermissionAction.READ, "attendance")
        ]
        ...
        self.enhanced_hr_permissions = [
            Permission.create("delete_users", "Delete user accounts", PermissionAction.DELETE, "user"),
            Permission.create("manage_payroll", "Manage payroll data", PermissionAction.MANAGE, "payroll"),
            Permission.create("execute_bulk_operations", "Execute bulk user operations", PermissionAction.EXECUTE, "user_bulk"),
            Permission.create("create_policies", "Create HR policies", PermissionAction.CREATE, "policy")
        ]
    ...
    def test_basic_role_inheritance_permissions(self):
        ...
        expected_count = len(self.default_rh_permissions) + len(self.enhanced_hr_permissions)
    ...
    def test_three_level_inheritance_chain(self):
        ...
        super_hr_permissions = [
            Permission.create("delete_organizations", "Delete organizations", PermissionAction.DELETE, "organization"),
            Permission.create("manage_system_settings", "Manage system settings", PermissionAction.MANAGE, "system"),
            Permission.create("execute_admin_commands", "Execute admin commands", PermissionAction.EXECUTE, "admin")
        ]
    ...
    def test_duplicate_permission_handling_in_inheritance(self):
        ...
        conflicting_permissions = [
            Permission.create("read_users", "Read user profiles (duplicate)", PermissionAction.READ, "user"),  # Duplicate
            Permission.create("new_permission", "A new unique permission", PermissionAction.CREATE, "new_resource")
        ]
        ...
        permission_keys = [f"{p.action.value}:{p.resource_type}" for p in effective_permissions]
        assert len(permission_keys) == len(set(permission_keys))  # No duplicates
        
        # Should have parent permissions + unique child permissions
        expected_unique_permissions = len(self.default_rh_permissions) + 1  # +1 for new_permission
    ...
    def test_circular_inheritance_detection(self):
        ...
        role_b = Role.create("Role_B", "Second role", self.creator_id, self.org_id, parent_role_id=role_a.id)
        
        # Try to make role_a inherit from role_b (creating a circle)
        all_roles = [role_a, role_b]
        
        can_inherit, reason = self.service.can_role_inherit_from(role_a, role_b, all_roles)
    ...
    def test_deep_inheritance_chain_with_cycles_prevention(self):
        ...
        can_inherit, reason = self.service.can_role_inherit_from(roles[0], deepest_role, roles)
        assert can_inherit is False
        assert "circular inheritance" in reason.lower()
    ...
    def test_global_to_organization_inheritance(self):
        ...
        can_inherit, reason = self.service.can_role_inherit_from(org_role, global_role, all_roles)
    ...
    def test_permission_override_precedence_in_inheritance(self):
        ...
        read_user_perms = [p for p in effective_permissions if p.resource_type == "user" and p.action == PermissionAction.READ]
    ...
    def test_multi_parent_simulation_through_composition(self):
        ...
        finance_role = Role.create("Finance_Base", "Base finance permissions", self.creator_id, self.org_id)
        hr_role = Role.create("HR_Base", "Base HR permissions", self.creator_id, self.org_id)
        
        # Create permissions for each
        finance_permissions = [
            Permission.create("read_finances", "Read financial data", PermissionAction.READ, "finance"),
            Permission.create("create_budgets", "Create budgets", PermissionAction.CREATE, "budget")
        ]
        
        hr_permissions = [
            Permission.create("manage_employees", "Manage employees", PermissionAction.MANAGE, "employee"),
            Permission.create("read_hr_reports", "Read HR reports", PermissionAction.READ, "hr_report")
        ]
    ...
    def test_role_hierarchy_with_missing_intermediate_roles(self):
        ...
        grandparent = Role.create("Grandparent", "Grandparent role", self.creator_id, self.org_id)
        parent = Role.create("Parent", "Parent role", self.creator_id, self.org_id, parent_role_id=grandparent.id)
        child = Role.create("Child", "Child role", self.creator_id, self.org_id, parent_role_id=parent.id)
    ...
    def test_permission_inheritance_with_large_role_tree(self):
        ...
        all_roles = [root_role]
        role_permissions = {root_role.id: [Permission.create("root_perm", "Root permission", PermissionAction.READ, "root")]}
        ...
        for level in range(3):
            ...
            for parent in current_level:
                for i in range(3):
                    child = Role.create(
                        f"Level_{level}_Child_{i}_{parent.name.value}",
                        f"Child {i} of {parent.name.value}",
                        self.creator_id,
                        self.org_id,
        ...
        expected_permission_count = len(hierarchy_path)  # One permission per role in path
    ...
    def test_role_deletion_impact_on_inheritance(self):
        ...
        role_b = Role.create("Role_B", "Role B", self.creator_id, self.org_id, parent_role_id=role_a.id)
        role_c = Role.create("Role_C", "Role C", self.creator_id, self.org_id, parent_role_id=role_b.id)
        
        all_roles = [role_a, role_b, role_c]
        
        # Verify initial hierarchy
        assert role_c.get_role_hierarchy_path(all_roles) == [role_a.id, role_b.id, role_c.id]
        ...
        assert any("Parent role" in error and str(role_b.id) in error for error in errors)
    ...
    def test_permission_conflicts_in_complex_inheritance(self):
        ...
        base_read_role = Role.create("Base_Reader", "Base reader role", self.creator_id, self.org_id)
        base_write_role = Role.create("Base_Writer", "Base writer role", self.creator_id, self.org_id)
        ...
        read_perm = Permission.create("access_data", "Read access to data", PermissionAction.READ, "data")
        write_perm = Permission.create("access_data_write", "Write access to data", PermissionAction.UPDATE, "data")
        
        all_roles = [base_read_role, base_write_role, composite_child]
        role_permissions = {
            base_read_role.id: [read_perm],
            composite_child.id: [write_perm]  # Child adds write permission to same resource
        }
        ...
        data_permissions = [p for p in effective_permissions if p.resource_type == "data"]
