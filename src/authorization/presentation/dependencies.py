...
from ..infrastructure.repositories.sqlalchemy_role_repository import SqlAlchemyRoleRepository
from ..infrastructure.repositories.sqlalchemy_permission_repository import SqlAlchemyPermissionRepository
...
def get_role_repository(db: Session = Depends(get_db_session)) -> SqlAlchemyRoleRepository:
    """Get role repository dependency."""
    return SqlAlchemyRoleRepository(db)


def get_permission_repository(db: Session = Depends(get_db_session)) -> SqlAlchemyPermissionRepository:
...
def get_role_use_case(
    ...
    permission_repo: SqlAlchemyPermissionRepository = Depends(get_permission_repository),
) -> RoleUseCase:
