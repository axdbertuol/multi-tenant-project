...
class ProfileUseCase:
    ...
    def create_profile(self, dto: ProfileCreateDTO) -> ProfileResponseDTO:
        ...
        if self.profile_repository.exists_by_name(dto.name, dto.organization_id):
            raise ValueError(
                f"Profile with name '{dto.name}' already exists in this organization"
            )
    ...
    def update_profile(
        self, profile_id: UUID, dto: ProfileUpdateDTO
    ) -> Optional[ProfileResponseDTO]:
        ...
        if dto.name is not None:
            ...
            if dto.name != profile.name and self.profile_repository.exists_by_name(
                dto.name, profile.organization_id
            ):
                raise ValueError(
                    f"Profile with name '{dto.name}' already exists in this organization"
                )
        ...
        if dto.profile_metadata is not None:
            updated_profile = updated_profile.update_profile_metadata(
                dto.profile_metadata
            )
    ...
    def delete_profile(self, profile_id: UUID) -> bool:
        ...
        active_assignments = self.user_profile_repository.get_active_by_profile(
            profile_id
        )
        if active_assignments:
            raise ValueError(
                f"Cannot delete profile with {len(active_assignments)} active user assignments"
            )
    ...
    def list_profiles(
        ...
        if filters:
            if filters.organization_id:
                if filters.is_active is not None:
                    ...
                    profiles = self.profile_repository.get_by_organization(
                ...
                if filters.is_system_profile:
                    ...
                    all_profiles = self.profile_repository.get_by_organization(
                        filters.organization_id
                    )
        ...
        if filters:
            if filters.name:
                profiles = [
                    p for p in profiles if filters.name.lower() in p.name.lower()
                ]
            if filters.has_users is not None:
                for profile in profiles:
                    user_count = self.user_profile_repository.count_active_by_profile(
                        profile.id
                    )
            ...
            if filters.has_permissions is not None:
                for profile in profiles:
                    permission_count = self.profile_folder_permission_repository.count_active_by_profile(
                        profile.id
                    )
    ...
    def get_profile_stats(self, organization_id: UUID) -> ProfileStatsDTO:
        ...
        for profile in all_profiles:
            ...
            if (
                self.profile_folder_permission_repository.count_active_by_profile(
                    profile.id
                )
                > 0
            ):
        ...
        for profile in all_profiles:
            ...
            profiles_by_creator[creator_name] = (
                profiles_by_creator.get(creator_name, 0) + 1
            )

        # Get recent profiles
        recent_profiles = self.profile_repository.get_recently_created(
            organization_id, days=30
        )
        recent_profile_responses = [
            self._build_profile_response(p) for p in recent_profiles
        ]
        ...
        for profile in all_profiles:
            ...
            if user_count > 0:
                most_used_profiles.append(
                    (self._build_profile_response(profile), user_count)
                )
    ...
    def validate_profile(
        ...
        if dto.validation_type in ["full", "permissions"]:
            ...
            if not permissions:
                issues.append(
                    {"type": "warning", "message": "Profile has no folder permissions"}
                )
                recommendations.append(
                    "Consider adding folder permissions to make this profile useful"
                )
        ...
        if dto.validation_type == "full":
            ...
            if not active_assignments:
                issues.append(
                    {
                        "type": "info",
                        "message": "Profile has no active user assignments",
                    }
                )
    ...
    def clone_profile(self, dto: ProfileCloneDTO) -> ProfileCloneResponseDTO:
        ...
        target_organization_id = (
            dto.target_organization_id or source_profile.organization_id
        )

        # Check if new name conflicts
        if self.profile_repository.exists_by_name(dto.new_name, target_organization_id):
            raise ValueError(
                f"Profile with name '{dto.new_name}' already exists in target organization"
            )
        ...
        cloned_profile = Profile.create(
            ...
            profile_metadata=source_profile.profile_metadata
            if dto.clone_profile_metadata
            else {},
        )
        ...
        if dto.clone_permissions:
            ...
            for perm in source_permissions:
                try:
                    cloned_permission = perm.model_copy(
                        update={
                            "id": uuid4(),
                    ...
                    warnings.append(
                        f"Failed to clone permission for {perm.folder_path}: {str(e)}"
                    )

        # Clone user assignments
        if dto.clone_users:
            source_assignments = self.user_profile_repository.get_active_by_profile(
                source_profile.id
            )
            for assignment in source_assignments:
                try:
                    ...
                    if target_organization_id == source_profile.organization_id:
                        cloned_assignment = assignment.model_copy(
                            update={
                                "id": uuid4(),
                                "profile_id": saved_cloned_profile.id,
                        ...
                        warnings.append(
                            f"Skipped cloning user assignment for different organization"
                        )
                except Exception as e:
    ...
    def get_profile_usage_stats(self, profile_id: UUID) -> ProfileUsageStatsDTO:
        ...
        # Get permissions
        permissions = self.profile_folder_permission_repository.get_active_by_profile(
            profile_id
        )
        ...
        usage_score = self._calculate_profile_usage_score(
            profile, active_assignments, permissions
        )
        ...
        if len(active_assignments) == 0:
            recommendations.append(
                "Profile has no active users - consider assigning users or archiving"
            )
        if len(permissions) == 0:
            recommendations.append(
                "Profile has no folder permissions - add permissions to make it useful"
            )
        if usage_score < 30:
            recommendations.append(
                "Low usage score - review if this profile is still needed"
            )
            ...
            try:
                ...
                if dto.action == "activate":
                    ...
                    if not can_delete:
                        failure_count += 1
                        errors.append(
                            f"Profile {profile_id} cannot be deleted: {reason}"
                        )
                        continue

                    # Check for active assignments
                    active_assignments = (
                        self.user_profile_repository.get_active_by_profile(profile_id)
                    )
                    if active_assignments:
                        failure_count += 1
                        errors.append(
                            f"Profile {profile_id} has {len(active_assignments)} active assignments"
                        )
                        continue

                    # Delete permissions first
                    self.profile_folder_permission_repository.delete_by_profile(
                        profile_id
                    )
    ...
    def _build_profile_detail_response(
    ...
    def _calculate_profile_usage_score(
        self, profile: Profile, active_assignments: list, permissions: list
    ) -> float:
