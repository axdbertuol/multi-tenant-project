...
class OrganizationSettingsUpdateDTO(BaseModel):
    ...
    max_users: Optional[int] = Field(
        None, ge=1, le=1000, description="Maximum users allowed"
    )
    allow_user_registration: Optional[bool] = Field(
        None, description="Allow user registration"
    )
    require_email_verification: Optional[bool] = Field(
        None, description="Require email verification"
    )
    session_timeout_hours: Optional[int] = Field(
        None, ge=1, le=720, description="Session timeout in hours"
    )
