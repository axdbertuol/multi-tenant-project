...
from .repositories.sqlalchemy_user_session_repository import SqlAlchemyUserSessionRepository
from .repositories.sqlalchemy_role_repository import SqlAlchemyRoleRepository
from .repositories.sqlalchemy_permission_repository import SqlAlchemyPermissionRepository
...
class IAMUnitOfWork(SQLAlchemyUnitOfWork):
    ...
    def __init__(self, session: Session, repositories: list[str]):
        ...
        if "permission" in repositories:
            self._repositories.update({"permission": SqlAlchemyPermissionRepository(session)})
        if "policy" in repositories:
            self._repositories.update({"policy": SqlAlchemyPolicyRepository(session)})
        if "resource" in repositories:
            self._repositories.update({"resource": SqlAlchemyResourceRepository(session)})
