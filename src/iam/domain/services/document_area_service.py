...
class DocumentAreaService:
    ...
    def assign_area_to_user(
        ...
        """
        Atribui uma área a um usuário (mantendo a função atual).
        ...
        # Obter atribuição atual do usuário
        ...
        # Atualizar área
        updated_assignment = current_assignment.change_area(area_id, assigned_by)
    ...
    def get_user_accessible_areas(
        ...
        primary_area = self.document_area_repository.get_by_id(assignment.area_id)
        if not primary_area:
            return []
    ...
    def setup_default_areas_for_organization(
        self,
        organization_id: UUID,
        created_by: UUID,
    ) -> List[DocumentArea]:
        """
        Configura as áreas padrão para uma organização.
    ...
    def _update_folder_for_area(self, area: DocumentArea) -> DocumentFolder:
        ...
        existing_folder = self.document_folder_repository.get_by_area(area.id)
    ...
    def get_area_statistics(
        ...
        users_in_area = self.user_function_area_repository.get_by_area(area_id)
        ...
        # Obter áreas filhas
        child_areas = self.document_area_repository.get_children(area_id)
