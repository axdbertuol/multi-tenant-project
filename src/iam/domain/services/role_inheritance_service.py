...
class RoleInheritanceService:
    ...
    def get_child_roles(
        self, parent_role_id: UUID, all_roles: List[Role]
    ) -> List[Role]:
    ...
    def get_descendant_roles(
        self, ancestor_role_id: UUID, all_roles: List[Role]
    ) -> List[Role]:
    ...
    def validate_role_hierarchy(self, roles: List[Role]) -> List[str]:
        ...
        for role in roles:
            if role.parent_role_id and role.parent_role_id not in role_ids:
                errors.append(
                    f"Role {role.name.value}: Parent role {role.parent_role_id} not found"
                )
    ...
    def get_effective_permissions_for_user_roles(
        ...
        for role_id in user_role_ids:
            ...
            role_perms = self.calculate_inherited_permissions(
    ...
    def can_role_inherit_from(
        self, child_role: Role, potential_parent_role: Role, all_roles: List[Role]
    ) -> tuple[bool, str]:
        """
